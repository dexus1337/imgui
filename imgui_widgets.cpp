...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if ( !(flags & ImGuiColorEditFlags_LargeColorOnly) )
    {
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (flags & ImGuiColorEditFlags_LargeColorOnly)
        {
            window->DC.CursorPos = ImVec2(pos.x + inputs_offset_x, pos.y);
            btn = ColorButton("##ColorButton", col_v4, flags, ImVec2(w_full, GetFrameHeight()));
        }
        else
        {
            window->DC.CursorPos = ImVec2(pos.x + button_offset_x, pos.y);
            btn = ColorButton("##ColorButton", col_v4, flags);
        }
